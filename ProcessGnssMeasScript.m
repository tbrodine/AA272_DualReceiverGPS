function [allGpsEph, gnssRaw, QOIs] = ProcessGnssMeasScript(dirName,prFileName)
c = 2.99792458e8;
...
%prFileName = 'gnss_log_2023_02_23_15_04_55.txt'; %with duty cycling, no carrier phase
...
% fctSeconds = 1e-3*double(gnssRaw.utcTimeMillis(end));
...
[gpsPvt,svXYZB,rhoM,Rx_time,sigma_rho, SVID] = GpsWlsPvt(gnssMeas,allGpsEph);
QOIs = table();
QOIs.Rx_time = Rx_time;
QOIs.rho = rhoM;
QOIs.X = svXYZB(:,1); QOIs.Y = svXYZB(:,2); QOIs.Z = svXYZB(:,3); 
QOIs.B = svXYZB(:,4).*c; 
QOIs.sigma_rho = sigma_rho;
QOIs.SVID = SVID;
